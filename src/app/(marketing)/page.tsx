...
const features: FeatureItem[] = [
  { icon: Car,      title: "Gestión de flota",          desc: "Controlá todos tus vehículos desde un solo lugar. Auto, moto, camioneta — sin límites." },
  { icon: Bell,     title: "Recordatorios automáticos", desc: "Alertas por fecha y por kilometraje. Nunca más te olvidés de un cambio de aceite." },
  { icon: FileText, title: "Documentación digital",     desc: "Seguro, VTV, manual de servicio. Todo guardado y con alertas de vencimiento." },
  { icon: Gauge,    title: "Historial completo",         desc: "Cada mantenimiento registrado con fecha, km, costo y proveedor. Trazabilidad total." },
  { icon: Shield,   title: "Privacidad garantizada",    desc: "Tus datos son tuyos. Almacenamiento seguro con acceso restringido a tu cuenta." },
  { icon: Zap,      title: "Rapidísimo",                desc: "Interfaz diseñada para registrar un mantenimiento en menos de 30 segundos." },
];

const socialProof = ["Sin tarjeta de crédito", "Setup en 2 minutos", "Gratis para siempre"] as const;

const mockStats = [
  { label: "Mis vehículos", value: "3" },
  { label: "Próximos",      value: "2" },
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="max-w-5xl mx-auto px-4 pt-24 pb-20 text-center">
        ...
        <div className="mt-8 flex items-center justify-center gap-3 flex-wrap">
          <Button asChild size="lg" className="bg-primary text-primary-foreground hover:bg-primary/90 font-semibold">
            <Link href="/sign-up">Comenzar ahora <ArrowRight className="w-4 h-4 ml-2" /></Link>
          </Button>
      ...
      <section className="max-w-5xl mx-auto px-4 pb-24">
        <div className="text-center mb-12">
          <h2 className="font-display text-2xl md:text-3xl font-bold text-foreground">Todo lo que necesitás</h2>
          <p className="text-sm text-muted-foreground mt-2">Pensado para conductores exigentes que quieren cuidar su inversión.</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {features.map((feature: FeatureItem) => (
            <div key={feature.title} className="group p-5 rounded-xl border border-border/60 bg-card hover:border-primary/20 hover:bg-card/80 transition-all duration-200">
              <div className="w-9 h-9 bg-secondary rounded-lg flex items-center justify-center mb-4 group-hover:bg-primary/10 transition-colors">
                <feature.icon className="w-[18px] h-[18px] text-muted-foreground group-hover:text-primary transition-colors" strokeWidth={2} />
              </div>
      ...
      <section className="border-t border-border/40">
        <div className="max-w-5xl mx-auto px-4 py-20 text-center">
          <h2 className="font-display text-2xl md:text-3xl font-bold text-foreground mb-4">Empezá a cuidar tu flota hoy</h2>
          <p className="text-sm text-muted-foreground mb-8 max-w-md mx-auto">Gratis, sin límite de vehículos. Registrate en segundos.</p>
          <Button asChild size="lg" className="bg-primary text-primary-foreground hover:bg-primary/90 font-semibold">
            <Link href="/sign-up">Crear mi cuenta gratis <ArrowRight className="w-4 h-4 ml-2" /></Link>
          </Button>
        </div>
      </section>
